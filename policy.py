...
class Policy():
    def __init__(self,
        ...
        self.logger = Logger(logname=os.path.join(log_path,name), now=datetime.utcnow().strftime("%b_%d_%H_%M_%S"))
    ...
    def update(self,unscaled_obs, actions, rewards,env_idx=-1, trainWeight=False):
        ...
        if len(self.trajectories) > self.batch_size:
            ...
            self.scaler.update(unscaled)  # update running statistics for scaling observations
            self.logger.log({'_{}_MeanReward'.format(self.name): np.mean([t['rewards'].sum() for t in self.trajectories]),
                        '_{}_steps'.format(self.name): unscaled.shape[0]/self.batch_size})
            ...
            self._log_batch_stats(observes, actions, advantages, disc_sum_rew, self.logger, self.episode)
            self.trpo_net.update(observes, actions, advantages, env_idx, self.logger, trainWeight=trainWeight)  # update policy
    ...
    def _log_batch_stats(self, observes, actions, advantages, disc_sum_rew, logger, episode):
