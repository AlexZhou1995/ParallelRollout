...
class TrpoNet(object):
    ...
    def _policy_nn(self):
        ...
        for i in range(self.n_ways):
            with tf.variable_scope('env_pi{}'.format(i), reuse=tf.AUTO_REUSE):
                ...
                self.weight_vars.append(tf.get_variable('mix_weight{}'.format(i),shape=(),dtype=tf.float32,initializer=tf.random_normal_initializer(0,1)))
    ...
    def update(self, observes, actions, advantages, idx, logger, trainWeight = False):
        ...
        for e in range(self.epochs):
            ...
            if idx==-1:
                ...
                if trainWeight:
                    self.sess.run(self.train_weight_op[idx], feed_dict)
                else:
                    self.sess.run(self.train_sub_ops[idx], feed_dict)
    ...
    def get_vars(self):
        ...
        env_vars = []
        for i in range(self.n_ways):
            env_vars.append(tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES,scope='env_pi{}'.format(i)))

        return {'avg_vars':avg_vars,'env_vars':env_vars}

    def set_vars(self,var_dict):
        avg_vars = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='average_pi')
        self.sess.run([l_p.assign(g_p) for l_p, g_p in zip(avg_vars, var_dict['avg_vars'])])

        for i in range(len(var_dict['env_vars'])):
            env_vars = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='env_pi{}'.format(i))
            self.sess.run([l_p.assign(g_p) for l_p, g_p in zip(env_vars, var_dict['env_vars'][i])])
